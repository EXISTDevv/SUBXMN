...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizesubxAmount"))
        settings.setValue("nAnonymizesubxAmount", 1000);
    nAnonymizesubxAmount = settings.value("nAnonymizesubxAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizesubxAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizesubxAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizesubxAmount:
            return QVariant(nAnonymizesubxAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizesubxAmount:
            nAnonymizesubxAmount = value.toInt();
            settings.setValue("nAnonymizesubxAmount", nAnonymizesubxAmount);
            emit AnonymizesubxAmountChanged(nAnonymizesubxAmount);
            break;
        default:
            break;
        }
