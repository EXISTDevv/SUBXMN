...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizesubxAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizesubxAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizesubxAmount = strAnonymizesubxAmount.remove(strAnonymizesubxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizesubxAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizesubxAmount*COIN) nMaxToAnonymize = nAnonymizesubxAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizesubxAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizesubxAmount));
        strAnonymizesubxAmount = strAnonymizesubxAmount.remove(strAnonymizesubxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizesubxAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
