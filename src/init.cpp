...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: subx.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: subxd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 31953)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 31944)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizesubxamount=<n> " + _("Keep N SUBX anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SUBX is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SUBX version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizesubxAmount = GetArg("-anonymizesubxamount", 0);
    if(nAnonymizesubxAmount > 999999) nAnonymizesubxAmount = 999999;
    if(nAnonymizesubxAmount < 2) nAnonymizesubxAmount = 2;
    ...
    LogPrintf("Anonymize SUBX Amount %d\n", nAnonymizesubxAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1SUBX+1000 == (.1SUBX+100)*10
       10SUBX+10000 == (1SUBX+1000)*10
    */
