...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if ((pindexPrev != NULL) && (pindexPrev->nHeight > Params().POSStartBlock()))
    {
        int64_t tmp_block = pindexPrev->nHeight % 20;
        int64_t mean = 500000 * COIN;

        if (tmp_block < 6)
        {
            nSubsidy = 100000 * COIN * tmp_block + mean;
        }
        else if (tmp_block < 16)
        {
            nSubsidy = -100000 * COIN * tmp_block + 3*mean;
        }
        else
        {
            nSubsidy = 100000 * COIN * tmp_block  - 3*mean;
        ...
        if(fDebug)
            LogPrintf("creation -> GetProofOfStakeReward(): create=%d(%s)[%d] tmp_block=%d\n",nSubsidy, FormatMoney(nSubsidy), pindexPrev->nHeight, tmp_block);
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        if(fDebug)
            LogPrintf("%s: nCalculatedStakeReward=%d(%s)[%d]  nStakeReward=%d(%s)[%d]\n",__func__, nCalculatedStakeReward, FormatMoney(nCalculatedStakeReward), pindex->pprev->nHeight,nStakeReward,  FormatMoney(nStakeReward), pindex->nHeight);
